...
public:
    one2many_bitmask_reader(content_allocator_ptr content_allocator, ring_buffer_t storage, std::size_t storage_mask, counter_t read_from, counter_t mask)
        : m_content_allocator(std::move(content_allocator))
        ...
        , m_reader_mask(mask)
    {
        static_assert(sizeof(one2many_bitmask_reader<event_t, counter_t, content_allocator_t>) <= QUEUE_CPU_CACHE_LINE_SIZE);
    }
...
{
public:
    using writer_type = one2many_bitmask_queue<event_t, counter_t, content_allocator_t>;
    using reader_type = one2many_bitmask_reader<event_t, counter_t, content_allocator_t>;
    using ring_buffer_t = one2many_bitmask_ring_buffer_t<event_t, counter_t>;
    using content_allocator_ptr = std::shared_ptr<content_allocator_t>;
...
public:
    one2many_bitmask_queue(std::size_t n, content_allocator_t content_allocator = content_allocator_t())
    ...
    {
        static_assert(sizeof(one2many_bitmask_queue<event_t, counter_t, content_allocator_t>) <= QUEUE_CPU_CACHE_LINE_SIZE);
    ...
    std::optional<reader_type> create_reader() noexcept
    {
        ...
        if (next_id <= one2many_bitmask_queue_impl<counter_t>::MAX_READER_ID)
        {
            ...
            return std::make_optional<reader_type>(m_content_allocator, m_storage, m_storage_mask, m_next_seq_num, mask);
        }
        else
        {
            return std::nullopt;
