...
#include "one2each_seqnum_bucket.h"
...
using one2each_seqnum_stream_ring_buffer_t = std::shared_ptr<impl::one2each_seqnum_bucket<event_t, counter_t>>;

template<typename event_t, typename counter_t>
class one2each_seqnum_stream_queue_impl final
{
public:
    using bucket_type = impl::one2each_seqnum_bucket<event_t, counter_t>;
    using ring_buffer_type = one2each_seqnum_stream_ring_buffer_t<event_t, counter_t>;
    ...
    one2each_seqnum_stream_queue_impl(std::size_t n, std::unique_ptr<CA, CD> content_allocator, RA region_allocator = RA())
    ...
    one2each_seqnum_stream_queue_impl(std::size_t n, RA region_allocator = RA())
    ...
    one2each_seqnum_stream_queue_impl(one2each_seqnum_stream_queue_impl&&) noexcept = default;

    one2each_seqnum_stream_queue_impl& operator=(one2each_seqnum_stream_queue_impl&&) noexcept = delete;
    one2each_seqnum_stream_queue_impl(const one2each_seqnum_stream_queue_impl&) = delete;
    one2each_seqnum_stream_queue_impl& operator=(const one2each_seqnum_stream_queue_impl&) = delete;
    ...
    bool try_write(event_t&& event, counter_t counter) noexcept
    {
        ...
        if (bucket.m_counter.load(std::memory_order_acquire) == counter_type::EMPTY_DATA_MARK)
        {
