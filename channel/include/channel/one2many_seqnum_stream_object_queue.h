...
// buffer
template<class event_t, typename counter_t>
using one2many_seqnum_stream_object_ring_buffer_t = std::shared_ptr<one2many_seqnum_bucket<event_t, counter_t>>;
...
template<class event_t, typename counter_t>
class alignas(QUEUE_CPU_CACHE_LINE_SIZE) one2many_seqnum_stream_object_reader final
{
public:
    using guard_type = one2many_seqnum_stream_object_guard<event_t, counter_t>;
    using ring_buffer_type = one2many_seqnum_stream_object_ring_buffer_t<event_t, counter_t>;
...
public:
    one2many_seqnum_stream_object_reader(ring_buffer_type storage, std::size_t storage_mask, counter_t read_from, counter_t id) noexcept
        ...
        , m_id(id)
    {
        static_assert(sizeof(one2many_seqnum_stream_object_reader<event_t, counter_t>) <= QUEUE_CPU_CACHE_LINE_SIZE);
    }
...
template<class event_t, typename counter_t>
class one2many_seqnum_stream_object_queue_impl final
{
public:
    using bucket_type = one2many_seqnum_bucket<event_t, counter_t>;
    ...
    one2many_seqnum_stream_object_queue_impl(std::size_t n, std::unique_ptr<A, D> content_allocator)
    ...
    one2many_seqnum_stream_object_queue_impl(std::size_t n)
...
class alignas(QUEUE_CPU_CACHE_LINE_SIZE) one2many_seqnum_stream_object_queue final : public impl::stream_object_allocator_holder<content_allocator_t>
{
public:
    using reader_type = one2many_seqnum_stream_object_reader<event_t, counter_t>;
...
public:
    ...
    {
        static_assert(sizeof(one2many_seqnum_stream_object_queue<event_t, counter_t, content_allocator_t>) <= QUEUE_CPU_CACHE_LINE_SIZE);
    }
    ...
    one2many_seqnum_stream_object_queue(std::size_t n, std::unique_ptr<content_allocator_t, Deleter> content_allocator)
...
private:
    impl::one2many_seqnum_stream_object_queue_impl<event_t, counter_t> m_impl;
};
