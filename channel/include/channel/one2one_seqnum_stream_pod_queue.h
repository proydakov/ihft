...
// buffer
template<class event_t, typename counter_t>
using one2one_seqnum_stream_pod_ring_buffer_t = std::shared_ptr<one2one_seqnum_bucket<event_t, counter_t>>;

// implementation

// reader
template<class event_t, typename counter_t>
class alignas(QUEUE_CPU_CACHE_LINE_SIZE) one2one_seqnum_stream_pod_reader final
{
public:
    using ring_buffer_t = one2one_seqnum_stream_pod_ring_buffer_t<event_t, counter_t>;
    using event_type = event_t;

public:
    // ctor
    one2one_seqnum_stream_pod_reader(ring_buffer_t storage, std::size_t storage_mask, counter_t read_from, counter_t id) noexcept
        ...
        , m_id(id)
    {
        static_assert(sizeof(one2one_seqnum_stream_pod_reader<event_t, counter_t>) <= QUEUE_CPU_CACHE_LINE_SIZE);
    }
    ...
    std::optional<event_t> try_read() noexcept
    {
        ...
        if (bucket.m_seqn.load(std::memory_order_acquire) == m_next_read_index)
        {
            ...
            bucket.m_seqn.store(impl::one2one_counter_queue_constant<counter_t>::DUMMY_EVENT_SEQ_NUM, std::memory_order_release);
            return opt;
        }
        else
        {
...
class alignas(QUEUE_CPU_CACHE_LINE_SIZE) one2one_seqnum_stream_pod_queue final
{
public:
...
public:
    ...
    {
        static_assert(sizeof(one2one_seqnum_stream_pod_queue<event_t, counter_t>) <= QUEUE_CPU_CACHE_LINE_SIZE);
